      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between pb-2 gap-4">
        ...
        <div className="flex items-center gap-2.5">
          <button
            ...
            className="rounded-xl border border-slate-205/65 dark:border-slate-800 px-3 py-2 text-3xs font-extrabold uppercase tracking-wide text-slate-505 dark:text-slate-350 hover:bg-slate-50 dark:hover:bg-slate-900/30 cursor-pointer"
          >
            ← Prev Week
          </button>
          <button
            onClick={() => setFocusDate(new Date())}
            className="rounded-xl border border-slate-205/65 dark:border-slate-800 px-4 py-2 text-xs font-bold text-slate-655 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-900/30 cursor-pointer"
          >
            Today
          </button>
          <button
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 xl:grid-cols-4 gap-6 items-stretch flex-1">
          ...
          <div className="xl:col-span-3 glass-card rounded-2xl flex flex-col overflow-hidden">
            ...
            <div className="flex bg-slate-50/50 dark:bg-slate-950/20 border-b border-slate-200/40 dark:border-slate-800/60 select-none">
            ...
            <div className="flex-1 overflow-y-auto divide-y divide-slate-100/50 dark:divide-slate-850/60 scrollbar-premium">
              {scheduledTasks.length === 0 ? (
                <div className="flex flex-col items-center justify-center text-slate-400 py-24 text-center">
                  ...
                  <h4 className="text-xs font-bold text-slate-750 dark:text-slate-300">No Scheduled Tasks</h4>
                  <p className="text-3xs text-slate-450 dark:text-slate-500 mt-1 max-w-[280px]">
                ...
                scheduledTasks.map((task) => {
                  ...
                  return (
                    <div key={task._id} className="flex min-h-[50px] items-stretch hover:bg-slate-50/20 dark:hover:bg-slate-900/5 transition-colors group">
                      ...
                      <div className="flex-1 min-w-[750px] relative select-none">
                        ...
                        <div className="absolute inset-y-0 py-2.5 left-0 right-0">
                          <div
                            className={`grid grid-cols-30 h-full w-full`}
                          >
                            <div
                              className={`rounded-lg py-0.5 px-2.5 text-4xs font-black truncate border flex items-center justify-between shadow-2xs group-hover:shadow-xs transition-all relative ${
                                task.status === 'Completed'
                                  ? 'bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 border-emerald-500/20 shadow-emerald-500/2'
                                  : task.priority === 'High'
                                  ? 'bg-rose-500/10 text-rose-600 dark:text-rose-450 border-rose-500/20 shadow-rose-500/2'
          ...
          <div className="xl:col-span-1 glass-card p-5 rounded-2xl flex flex-col h-full overflow-hidden">
            ...
            <div className="flex-1 overflow-y-auto space-y-3.5 pr-0.5 scrollbar-premium">
              {unscheduledTasks.length === 0 ? (
                <p className="text-center text-3xs text-slate-400 italic py-8">No backlog items.</p>
              ) : (
                unscheduledTasks.map((task) => (
                  <div
                    key={task._id}
                    className="p-3 rounded-xl bg-white/40 dark:bg-slate-950/20 border border-slate-200/50 dark:border-slate-850/65 flex flex-col gap-2 hover:border-emerald-500/20 transition-all duration-200"
                  >
                    ...
                    <div className="flex items-center justify-between mt-1">
                      ...
                      <div className="flex items-center gap-1.5 text-4xs font-bold text-slate-405">
